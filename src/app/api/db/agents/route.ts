...
export async function GET() {
  try {
    ...
    `);
    const tools = await query(`SELECT * FROM agent_tools ORDER BY id`);
    return NextResponse.json(agents.map((a: any) => ({
      ...a,
      tools: tools
        .filter((t: any) => t.agent_id === a.id)
        .map((t: any) => ({ id:t.id, name:t.name, description:t.description, enabled:t.enabled })),
    })));
  } catch (e: any) {
...
export async function POST(req: NextRequest) {
  try {
    ...
    `, [id,name,roomId,role,purpose,color,charModel||'knight_m',
        ...
        tasksCompleted||0,tasksErrored||0,createdAt]);
    if (tools?.length) {
      ...
      for (const t of tools) {
        await query(`INSERT INTO agent_tools (id,agent_id,name,description,enabled) VALUES ($1,$2,$3,$4,$5)`,
          [t.id,id,t.name,t.description||'',t.enabled]);
      }
    }
