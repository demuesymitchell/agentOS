...
import {
  seedIfEmpty, getRooms, saveRooms, getAgents, saveAgents,
  getTasks, saveTasks, getLogs, saveLogs, resetAll,
} from './db/persist';

const DEFAULT_PANELS: PanelState[] = [
  { id: 'terminal',     label: 'TERMINAL',      open: true,  minimized: false },
  { id: 'tasks',        label: 'TASKS',         open: true,  minimized: false },
  { id: 'media',        label: 'MEDIA OUTPUT',  open: false, minimized: false },
  { id: 'agentInspect', label: 'AGENT INSPECT', open: false, minimized: false },
  { id: 'admin',        label: 'ADMIN',         open: true,  minimized: false },
];
...
export const useStore = create<Store>((set, get) => ({
  ...
  goalInput: '', isProcessing: false,

  // ── Hydrate ──────────────────────────────────────────────────────────────────
  hydrate() {
    const { rooms, agents } = seedIfEmpty();
    set({ rooms, agents, tasks: getTasks(), logs: getLogs() });
  },
  ...
  addRoom(data) {
    ...
    set(s => { const rooms = [...s.rooms, room]; saveRooms(rooms); return { rooms }; });
    return room;
  },
  updateRoom(id, patch) {
    set(s => { const rooms = s.rooms.map(r => r.id === id ? { ...r, ...patch } : r); saveRooms(rooms); return { rooms }; });
  },
  deleteRoom(id) {
    set(s => { const rooms = s.rooms.filter(r => r.id !== id); saveRooms(rooms); return { rooms }; });
  },

  // ── Agents ───────────────────────────────────────────────────────────────────
  addAgent(data) {
    const agent: Agent = { ...data, id: uuid(), status: 'idle', currentTask: null, tasksCompleted: 0, tasksErrored: 0, createdAt: Date.now(), charModel: data.charModel || 'knight_m' };
    set(s => { const agents = [...s.agents, agent]; saveAgents(agents); return { agents }; });
    return agent;
  },
  updateAgent(id, patch) {
    set(s => { const agents = s.agents.map(a => a.id === id ? { ...a, ...patch } : a); saveAgents(agents); return { agents }; });
  },
  deleteAgent(id) {
    set(s => { const agents = s.agents.filter(a => a.id !== id); saveAgents(agents); return { agents }; });
  },

  // ── Tasks ────────────────────────────────────────────────────────────────────
  addTask(task) { set(s => { const tasks = [task, ...s.tasks]; saveTasks(tasks); return { tasks }; }); },
  updateTask(id, patch) { set(s => { const tasks = s.tasks.map(t => t.id === id ? { ...t, ...patch } : t); saveTasks(tasks); return { tasks }; }); },
  addOutput(taskId, output) {
    set(s => { const tasks = s.tasks.map(t => t.id === taskId ? { ...t, outputs: [...t.outputs, output] } : t); saveTasks(tasks); return { tasks }; });
  },

  // ── Logs ─────────────────────────────────────────────────────────────────────
  addLog(entry) {
    set(s => { const logs = [{ ...entry, id: uuid(), ts: Date.now() }, ...s.logs].slice(0, 300); saveLogs(logs); return { logs }; });
  },
  ...
  async submitGoal(goal: string) {
    ...
    // Find manager/director agent
    const mgmtRoom = rooms.find(r => r.name.toLowerCase().includes('manage'));
    ...
    if (!leader) {
      addLog({ level: 'error', from: 'OS', message: 'No agents found. Create agents in the Admin panel first.' });
    ...
    try {
      ...
      addLog({ level: 'success', from: leader.name, message: data.thought || 'Directive analyzed. Delegating...' });
      updateAgent(leader.id, { status: 'idle', currentTask: null });
      ...
      for (const sub of subtasks) {
        const room = rooms.find(r => r.name.toLowerCase().includes(sub.roomName.toLowerCase()))
          || rooms.find(r => r.id !== mgmtRoom?.id);
        if (!room) continue;

        const agent = agents.find(a => a.roomId === room.id) || null;

        if (!agent) {
          addLog({ level: 'warn', from: leader.name, to: room.name, message: `No agent in ${room.name} — skipping: ${sub.task}` });
          continue;
        }
        ...
        setTimeout(async () => {
          ...
          try {
            const r = await fetch('/api/agent', {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({ task: sub.task, agent: { name: agent.name, role: agent.role, purpose: agent.purpose, tools: agent.tools }, apiKey: agent.apiKeyOverride || null }),
            });
            ...
            updateAgent(agent.id, { status: 'idle', currentTask: null, tasksCompleted: (get().agents.find(a=>a.id===agent.id)?.tasksCompleted||0)+1 });
          } catch (e: any) {
            updateTask(taskId, { status: 'error', error: e?.message });
            addLog({ level: 'error', from: agent.name, message: `Failed: ${sub.task.slice(0, 50)}` });
            updateAgent(agent.id, { status: 'error', currentTask: null, tasksErrored: (get().agents.find(a=>a.id===agent.id)?.tasksErrored||0)+1 });
          }
