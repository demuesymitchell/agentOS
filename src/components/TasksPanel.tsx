...
const SS:Record<string,{c:string;i:string}>={
  queued:{c:'#4a3820',i:'○'},running:{c:'#e87830',i:'◉'},done:{c:'#50a060',i:'●'},error:{c:'#c83030',i:'✕'},
};
    ...
    <div className="mt-1">
      ...
      {o.content
        ...
        : <div className="py-3 text-center" style={{background:'#080604',border:'1px dashed #3a2818'}}>
            <p className="cinzel dim" style={{fontSize:11}}>🎨 Connect DALL-E to conjure</p>
          </div>}
    </div>
  );
  return (
    <div className="mt-1 p-2" style={{background:'#080604',border:'1px solid #3a2818'}}>
      ...
      <p className="cinzel parchment leading-snug whitespace-pre-wrap break-all" style={{fontSize:12}}>{o.content}</p>
    </div>
  );
}
...
function TaskCard({task}:{task:Task}) {
  ...
  const elapsed=task.completedAt&&task.startedAt?`${((task.completedAt-task.startedAt)/1000).toFixed(1)}s`:null;
  return (
      ...
      <button className="w-full flex items-start gap-2 p-2 text-left hover:bg-[#140e08] transition-colors"
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-0.5">
            <span className="cinzel font-semibold" style={{color:room?.color||'#e8c84a',fontSize:10}}>{room?.name||'?'}</span>
      ...
      {exp&&(
        <div className="px-2 pb-2 border-t border-[#1e1208]">
          <div className="grid grid-cols-3 gap-1 my-1">
            {[{l:'STATUS',v:task.status,c:s.c},{l:'OUTPUTS',v:task.outputs.length||'null',c:'#e8c84a'},{l:'TIME',v:elapsed||'null',c:'#4a3820'}].map(({l,v,c})=>(
              <div key={l} style={{background:'#080604',border:'1px solid #2a1e12'}} className="p-1">
                <p className="cinzel dim" style={{fontSize:8}}>{l}</p>
                <p className="cinzel" style={{fontSize:12,color:String(v)==='null'?'#3a2818':c}}>{String(v)}</p>
              </div>
            ))}
          </div>
          {task.outputs.length>0
            ...
            : task.status==='error'
              ...
              : <div className="p-2 text-center"><p className="cinzel dim" style={{fontSize:12}}>
...
export default function TasksPanel() {
  ...
  return (
    <Panel id="tasks" title="QUEST LOG" icon="📜"
      ...
      <div className="scroll-dungeon" style={{maxHeight:200,overflowY:'auto'}}>
        {filtered.length===0
