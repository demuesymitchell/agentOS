    ...
    <Panel id="terminal" title="COMMAND HALL" icon="⚔"
      ...
      <div ref={logRef} className="scroll-dungeon flex-1 p-2 space-y-0.5 min-h-0"
        ...
        {logs.slice(0,100).map(entry => {
          ...
          return (
            <div key={entry.id} className="flex gap-2 items-baseline log-entry cinzel leading-snug" style={{ fontSize:12 }}>
      ...
      <div className="flex-shrink-0 p-2 border-t border-[#3a2818]">
        ...
        <div className="flex items-center gap-2 px-2 py-1.5"
          ...
          <span className="gold flex-shrink-0 cinzel" style={{ fontSize:16 }}>{isProcessing ? '⟳' : '›'}</span>
          <input
            value={goalInput} onChange={e => setGoalInput(e.target.value)} onKeyDown={onKey}
          ...
          />
          <button
            onClick={() => goalInput.trim() && !isProcessing && has && submitGoal(goalInput.trim())}
            disabled={isProcessing || !goalInput.trim() || !has}
            className="btn-dungeon primary flex-shrink-0 text-[10px]">SEND</button>
        </div>
